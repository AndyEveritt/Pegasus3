...
from frontier_exploration.msg import ExploreTaskActionGoal
from frontier_exploration.msg import ExploreTaskActionResult
...
# Universal controls
CONTROL_SCHEME_CHANGE_BUTTON = 6
...
control_states = ["drive", "arm", "autonomous"]
...
def interpreter():
    ...
    state_pub = rospy.Publisher("control_state", String, queue_size=5)
    target_vel_pub = rospy.Publisher("rover_target_vel", Twist, queue_size=5)
    arm_command_pub = rospy.Publisher("rover_arm_command", ArmCommand, queue_size=5)
    ...
    autonomous_cancel_pub = rospy.Publisher("move_base/cancel", GoalID, queue_size=5)
    exploration_cancel_pub = rospy.Publisher("explore_server/cancel", GoalID, queue_size=5)
    ...
    while not rospy.is_shutdown():
        ...
        if(control_states[current_control_state_index] == "drive"):
            ...
            static_arm = ArmCommand()
            arm_command_pub.publish(static_arm)
            cancel_command = GoalID()
            autonomous_cancel_pub.publish(cancel_command)
            exploration_cancel_pub.publish(cancel_command)        
            autonomous_active = False
                
        elif(control_states[current_control_state_index] == "arm"):
            arm_command_pub.publish(arm_command)
            static_vel = Twist()
            target_vel_pub.publish(static_vel)
            cancel_command = GoalID()
            autonomous_cancel_pub.publish(cancel_command)
            exploration_cancel_pub.publish(cancel_command)
            autonomous_active = False
        
        elif(control_states[current_control_state_index] == "autonomous"):
            static_arm = ArmCommand()
            arm_command_pub.publish(static_arm)
        ...
        state_pub.publish(control_states[current_control_state_index])
...
def get_commands():
    ...
    if input_buttons[CONTROL_SCHEME_CHANGE_BUTTON] == 1 and switch_button_prev_state == 0:
        ...
        current_control_state_index = current_control_state_index % len(control_states)  # Ensure that result is within index
    ...
    try:
        if control_states[current_control_state_index] == "drive":
            ...
            linear_command, angular_command, servo_command, _ = drive_interpreter(input_axes, input_buttons)
...
def drive_interpreter(axes, buttons):
    ...
    # Input to cancel route planning
    toggle_autonomous = buttons[TOGGLE_AUTONOMOUS_BUTTON]
    ...
    return linear_command, angular_command, servo_command, toggle_autonomous
...
def initialise_subscriber():
    ...
    rospy.Subscriber("move_base/current_goal", PoseStamped, recieved_goal)
    rospy.Subscriber("explore_server/goal", ExploreTaskActionGoal, recieved_goal)
    rospy.Subscriber("move_base/result", MoveBaseActionResult, achieved_goal)
    rospy.Subscriber("explore_server/result", ExploreTaskActionResult, achieved_goal)
...
def recieved_goal(data):
    global autonomous_active
    autonomous_active = True

def achieved_goal(data):
    global autonomous_active
    autonomous_active = False
