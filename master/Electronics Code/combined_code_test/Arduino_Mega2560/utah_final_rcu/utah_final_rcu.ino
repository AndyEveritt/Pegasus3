...
void update_arm(){
  ...
  for (int i = 0; i < 6; i++){
    ...
    analogWrite(arrArmPWMPins[i], Overflow(4 * abs(arrArmPWM[i] - 128)));
  }
}
...
void update_drive(){
  for(int i = 0; i < 6; i++)
  {
    if(i < 3)
    {
      arrDrivePWM[i] = (int) (arrDriveMessage[0] - (arrDriveMessage[1] - 100));
    }
    else
    {
      arrDrivePWM[i] = (int) (arrDriveMessage[0] + (arrDriveMessage[1] - 100));
  ...
  // set PWM duty cycle for each wheel
  for (int i = 0; i < 6; i++){
    analogWrite(arrDrivePWMPins[i], Overflow(abs(arrDrivePWM[i] - 100)));
    Serial.println(Overflow(abs(arrDrivePWM[i] - 100)));
  }
}
